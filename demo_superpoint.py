...
class SuperPointFrontend(object):
  ...
  def run(self, img):
    ...
    if self.cuda:
      inp = inp.cuda()
    # Forward pass of network. 网络前向传播，获取输出的两部分：特征图和粗描述符
    outs = self.net.forward(inp)
    semi, coarse_desc = outs[0], outs[1]
    # Convert pytorch -> numpy. 将PyTorch张量转换为NumPy数组并去除多余的维度
    semi = semi.data.cpu().numpy().squeeze()
    # --- Process points. 应用Softmax函数并归一化
    dense = np.exp(semi) # Softmax.
    dense = dense / (np.sum(dense, axis=0)+.00001) # Should sum to 1.
    # Remove dustbin. 去除"dustbin"
    nodust = dense[:-1, :, :]
    # Reshape to get full resolution heatmap. 调整形状以获得完整分辨率的热图
    Hc = int(H / self.cell)
    Wc = int(W / self.cell)
    ...
    pts = np.zeros((3, len(xs))) # Populate point data sized 3xN.创建关键点数据
    pts[0, :] = ys
    pts[1, :] = xs
    pts[2, :] = heatmap[xs, ys]
    # 应用非极大值抑制
    ...
    # Remove points along border. 移除靠近边缘的点
    bord = self.border_remove
    toremoveW = np.logical_or(pts[0, :] < bord, pts[0, :] >= (W-bord))
    toremoveH = np.logical_or(pts[1, :] < bord, pts[1, :] >= (H-bord))
    toremove = np.logical_or(toremoveW, toremoveH)
    pts = pts[:, ~toremove]
    # --- Process descriptor. 描述符处理
    D = coarse_desc.shape[1]
    if pts.shape[1] == 0:
      desc = np.zeros((D, 0))
    else:
      # Interpolate into descriptor map using 2D point locations. 插值获取描述符
  ...
  def run_change(self, img, weights):
    """ Process a numpy image to extract points and descriptors.
    Input
      img - HxW numpy float32 input image in range [0,1].
    Output
    ...
    nodust = nodust.transpose(1, 2, 0)
    heatmap = np.reshape(nodust, [Hc, Wc, self.cell, self.cell])
    # assert weights.shape == heatmap.shape # 确保权重数组的形状与heatmap相同
    heatmap = np.transpose(heatmap, [0, 2, 1, 3])
    heatmap = np.reshape(heatmap, [Hc*self.cell, Wc*self.cell])
    heatmap *= weights

    # TODO 根据置信度阈值提取关键点 conf_thresh需要降低，这里做个实验吧（0.1-0.9）
