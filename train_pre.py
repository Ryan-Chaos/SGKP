...
from lib.datatsets_pre import *
from lib.utils import *
from lib.exceptions import *


# Argument parsing
parser = argparse.ArgumentParser(description="Training FeatureBooster using pre-extracted COCO or MegaDepth dataset")
...
parser.add_argument(
    '--dataset_path', type=str, default='D:/work/data/SECOND_train_set/output', nargs='+',
...
parser.add_argument( # resume
    '--initial_checkpoint', type=str, default=Path(__file__).parent / str("runs/megacoco_k+_SuperPoint+Boost-F_bs16_ep50_lr0.001_adamw_cos_lboost10_warmup500/checkpoints/epoch50.pth"),
...
# Log
suffix = 'megacoco_k+_{}_bs{}_ep{}_lr{}'.format(args.descriptor, args.batch_size, args.num_epochs, args.lr)
...
def process_epoch(data_loader, booster, optimizer, epoch_idx, train=True):
    ...
    for batch_idx, data in progress_bar:
        ...
        match_loss = torch.tensor(np.array([0], dtype=np.float32), device=device) # 匹配损失
        n_valid_samples = 0 # 有效样本数
        ...
        for idx_in_batch in range(b):
            # Annotations 从data中获取关键信息，例如关键点、描述符、距离等
            kps1 = data['kps1'][idx_in_batch].to(device) # kps1 和 kps2: 这些是关键点的坐标，分别来自数据批次中的第一组和第二组图像
            normalized_kps1 = data['normalized_kps1'][idx_in_batch].to(device) # 这些是归一化后的关键点坐标
            descs1 = data['descs1'][idx_in_batch].to(device) # 这些是描述符，它们是关键点的数值表示，用于在不同图像之间匹配关键点
            pos_dist1 = data['pos_dist1'][idx_in_batch].to(device) # 这些变量包含了关键点之间的距离信息，用于确定哪些关键点是匹配的（正样本）和不匹配的（负样本）
            ids1 = data['ids1'][idx_in_batch].to(device) # 这些是索引，用于从关键点和描述符的完整集合中选择特定的子集
            ids1 = ids1.long()
            ...
            pos_dist2 = data['pos_dist2'][idx_in_batch].to(device)
            ids2 = data['ids2'][idx_in_batch].to(device)
            ids2 = ids2.long()

            # 使用特征增强模型（booster模型）对描述符进行增强 ——————核心
            ...
            # Get the matching distance and label os_radius 和 neg_radius
            # 分别定义了正样本和负样本的距离阈值。如果两个关键点的距离小于或等于pos_radius，它们被视为正样本；如果大于或等于neg_radius，则被视为负样本
            pos_radius = 3
            neg_radius = 15
            
            # Find kps1 correspondences
            descriptors1_rf = descs1_refine[ids1, :]
            ids_has_gt = (pos_dist1 <= pos_radius).sum(dim=1) >= 1 # 一个布尔数组，表示每个ids1中的关键点是否有对应的正样本
            descriptors1_rf = descriptors1_rf[ids_has_gt, :]

            if descriptors1_rf.shape[0] == 0: # 如果descs1_refine中没有有效的描述符，则跳过当前循环迭代
                continue
            num_ids_has_gt += descriptors1_rf.shape[0] # 累加有效描述符的数量

            pos_dist1 = pos_dist1[ids_has_gt]
            # pos_labels 和 neg_labels 是基于pos_dist1和定义的半径计算出的正负样本标签
            pos_labels = (pos_dist1 <= pos_radius).float()
            neg_labels = (pos_dist1 >= neg_radius).float()

            descriptors1 = descs1[ids1, :]
            descriptors1 = descriptors1[ids_has_gt, :]
            # origin_ap_loss_ 和 boost_ap_loss_ 分别计算原始描述符和提升后描述符的平均精度损失（AP loss）
            ...
            descriptors2_rf = descs2_refine[ids2, :]
            ...
            pos_dist2 = pos_dist2[ids_has_gt]
            pos_labels = (pos_dist2 <= pos_radius).float()
            neg_labels = (pos_dist2 >= neg_radius).float()

            descriptors2 = descs2[ids2, :]
...
# set default tensor data type 设置默认张量数据类型
torch.set_default_tensor_type(torch.FloatTensor)

# descriptor to be boosted feature即为要增强的描述符
feature = args.descriptor
...
# Initialize the history 历史记录初始化
train_loss_history = []
validation_loss_history = []
start_epoch = 1
min_train_loss = 500
# load initial checkpoint if needed 检查点加载
if args.initial_checkpoint is not None:
    checkpoint = torch.load(args.initial_checkpoint)
    # checkpoint = Path(__file__).parent / str("runs/megacoco_k+_SuperPoint+Boost-F_bs16_ep50_lr0.001_adamw_cos_lboost10_warmup500/checkpoints/epoch50.pth")
...
for epoch_idx in range(start_epoch, args.num_epochs + 1):
    ...
    train_loss_history.append(
        process_epoch(training_dataloader, booster, optimizer, epoch_idx) # 核心
    )
    ...
    if (train_loss_history[-1] < min_train_loss):
        min_train_loss = train_loss_history[-1]
